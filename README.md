...
**注:** 所有 bot 管理员指令支持私聊且可以在指令最后 **加上群号** 来配置指定群聊

```shell
1. /gmc <on/off> (?群号) : 在群聊中(开启/关闭)群管插件
2. /gmc prefix <前缀> (?群号) : 修改当前群聊的功能触发前缀
3. /gmc dt (?群号) : 查看当前群聊所有配置项

4. /gmc <模块名> <on/off> (?群号) : 在当前群聊(开启/关闭)模块功能

5. /gmc <模块名> dt (?群号) : 查看模块所有配置项
6. /gmc <模块名> list (?群号) : 查看模块的权限组

7. /gmc <模块名> + <角色> (?群号) : 将角色添加到模块的权限组中
8. /gmc <模块名> - <角色> (?群号) : 将角色从模块的权限组中移除

9. /gmc <模块名> at <on/off> (?群号) : (开启/关闭)模块的at功能
...
access模块:
    ...
    ***黑名单也同理***
    ...
    4. /查看分管 (?群号) : 查看本群分管(分管可接受加群请求的处理结果)
    5. /分管+ <qq号/@某人> (?群号) : 将目标添加至本群分管
    6. /分管- <qq号/@某人> (?群号) : 将目标从本群分管移除
    7. /查看黑名单 (?群号) : 查看本群黑名单
    8. /黑+ <qq号/@某人> (?群号) : 将目标加入本群黑名单
    9. /黑- <qq号/@某人> (?群号) : 将目标从本群黑名单移除
    10. /拒绝理由 <理由> (?群号) : 设置群黑名单默认拒绝理由
...
#### 更新日志
-   1.4.2 [#issue9](https://github.com/Linxaee/kivibot-plugin-group-manager/issues/9)
    -   access模块基本指令支持私聊
-   1.4.1 [#issue9](https://github.com/Linxaee/kivibot-plugin-group-manager/issues/9)
    -   bot 管理员指令支持私聊并指定群号
    -   所有词条/分管/黑名单 指令支持私聊
    -   修复部分模块权限错误
